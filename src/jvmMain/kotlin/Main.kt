...
import com.gabrielleeg1.bedrockvoid.server.MinecraftPlayer
import com.gabrielleeg1.bedrockvoid.server.PlayerState
...
private val ctx = SupervisorJob() +
  CoroutineName("bedrock-void") +
...
fun handleLogin(
  ...
  player: MutableStateFlow<MinecraftPlayer?>,
): Flow<InboundPacket> = session.inboundPacketFlow.onEach { packet ->
  when (packet) {
    is LoginPacket -> {
      val data = packet.chainData.chain.third.extraData
      ...
      try {
        ...
        player.emit(MinecraftPlayer(data.displayName, session, packet.protocolVersion))
...
fun handleResourcePack(
  ...
  .onEach { packet ->
    when (packet.status) {
      ...
      Completed -> {
        player.update { it.copy(state = PlayerState.Joining) }
...
fun handleJoinGamePackets(
  ...
  when (packet) {
    is RequestChunkRadiusPacket -> {
      ...
      player.update { it.copy(state = PlayerState.InGame) }
    }
    is TickSyncPacket -> {
    }
  }
...
private val startGamePacket = StartGamePacket(
...
private val biomeDefinitionListNbt = Base64.getDecoder().decode(BIOME_DEFINITION_LIST.toByteArray())
